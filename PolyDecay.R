...
rarh1 = function(n , Sigma){
   ...
   # Psi operator with polynomial decay
   b = 0.8^(1:10)
   Psi.i = function(i){
      v1 = rnorm(D, 0, Sigma)
      v2 = rnorm(D, 0, Sigma)
      Psi = v1 %*% t(v2)
      b[i]* Psi/(base::norm(Psi, type="2"))
   }
   ...
   # recursion
   for (i in 11:(n+l_burnin)){
      coef[,i] = rowSums(sapply(1:10, function(k) Psi.i(k)%*%data[,i-k])) + data[,i]
   }
...
save(ds, er.ffpe, er.new, d.hat, p.hat , file="Poly.Slow.100.kappa08.rda")
